...
flow = 'WC'
table_wc = pd.read_table(os.path.join(dirname, flow, stddirs, chip+'_filtered_anyRNA_sorted.annotated.abbrev.CPfitted'), index_col=0)
variant_table_wc = pd.read_table(os.path.join(dirname, flow, stddirs, chip+'_filtered_anyRNA_sorted.annotated.perVariant.CPfitted'), index_col=0)
table_wc.dropna(axis=0, subset=['variant_number'], inplace=True)

concentrations = np.array([   0.91358025,    8.22222222,   24.66666667, 74., 222.,  666.])
flow = 'WC_9bp'
table = pd.read_table(os.path.join(dirname, flow, stddirs, chip+'_filtered_anyRNA_sorted.annotated.abbrev.CPfitted'), index_col=0)
variant_table = pd.read_table(os.path.join(dirname, flow, stddirs, chip+'_filtered_anyRNA_sorted.annotated.perVariant.CPfitted'), index_col=0)
...
#plot scatterplot
tectoData.plotScatterplot([variant_table_wc, variant_table])
...
# plot signal
tectoData.plotDeltaAbsFluorescence(a, filterName, concentrations=concentrations)
