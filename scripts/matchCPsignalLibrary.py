...
def findSeqMapDict(cpsignal,barcodeMap, is_designed):
    ...
    for name in  [col for col in cpsignal]:
        seqMap[name] = np.array(cpsignal[name])
    ...
    for name in [col for col in barcodeMap]:
        seqMap[name].iloc[indices_cpsignal] = np.array(barcodeMap[name].iloc[indices_barcode])
    return seqMap
...
if __name__ == '__main__':
    ...
    cpsignal.sort('index1_seq', inplace=True)
