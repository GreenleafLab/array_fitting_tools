...
def fitSingleCurve(x, fluorescence, fitParameters, func,
    ...
    # 20170814: Ben : Heuristic to determine if cluster is 'non-binder'?
    # What's a fast/stable way to determine if signal doesn't increase?
    # If the average of the last three points is not at least 50% greater 
    # than the average of the first 3 points, don't try to fit.

    begin_avg = np.mean(fluorescence[:3])
    end_avg = np.mean(fluorescence[-3:])
    if end_avg <= 1.5*begin_avg:
        # Signal does not significantly increase. Call it a non-binder    
        return nonBinderParams(final_params, param_names)
    ...
    # If first check failed, make sure fit fmax is significantly higher than 
    # fit fmin: (100%?)
    if params['fmax'] < 2*params['fmin']:
        return nonBinderParams(final_params, param_names)
...
def nonBinderParams(final_params, param_names):
    # 20170814: forced params for a non-binding cluster
    for param in param_names:
        final_params.loc[param] = -1
        final_params.loc['%s_stde'%param] = 0
    final_params.loc['rsq'] = 1.0
    final_params.loc['exit_flag'] = -1
    final_params.loc['rmse'] = 0
    return final_params
...
def perCluster(concentrations, fluorescence, fitParameters, plot=None, change_params=None, func=None,
    ...
    try:
        ...
        single = fitSingleCurve(concentrations, fluorescence, fitParameters, func, kwargs=kwargs)
